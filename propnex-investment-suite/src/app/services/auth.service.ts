...
export class AuthService {
  ...
  fetchFBUsers() {
    return this.http
      .get(
        'https://propnexuser-default-rtdb.asia-southeast1.firebasedatabase.app/.json'
      )
      .pipe(
        map(resData => {
          const users = [];
  ...
  addUser(email: string, name: string, password: string) {
    ...
    return this.http
      .post('https://propnexuser-default-rtdb.asia-southeast1.firebasedatabase.app/.json',
      { ...newUser })
      .pipe(
        switchMap(() => {
          return this.fbUsers;
    ...
    return this.fbUsers.pipe(
      ...
      switchMap(users => {
        ...
        return this.http.put(
          `https://propnexuser-default-rtdb.asia-southeast1.firebasedatabase.app/${updatedUserIndex}.json`,
  ...
  removeFav(userInd: number, placeInd: number) {
    return this.http
      .delete(
        `https://propnexuser-default-rtdb.asia-southeast1.firebasedatabase.app/${userInd}/favourites/${placeInd}.json`
      );
  }
}
